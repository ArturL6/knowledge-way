...
| **benchmark-run** | Hermes (cheap tier) | State-based: **(a)** Stage 0 harness not yet built → no-op; **(b)** harness exists and integration head changed since last scorecard → run the retrieval scorecard, commit results; **(c)** external comparator comparison harness exists (packet 0.4 makes it scripted) → run both sides — our system and external comparator — on the same gold tasks and commit the comparative scorecard; **(d)** any regression vs the previous scorecard → write a drift flag for Sol. |
...
This applies to: embedding generation (1.3/1.4), card generation (1.4, 3.3, 3.4), the LLM query-planner fallback (1.6), rerank evaluation (1.8), and any external comparator chat-mode evaluation. Deterministic indexing (tree-sitter, FTS, graph) is exempt.
...
- Packet **0.4** is upgraded: don't just test-drive external comparator once — **script it**. Deliver a repeatable headless runner (external comparator CLI, keyless core; its optional chat mode via the existing key, subset-first per §4) that answers the 12 question classes on the gold-task repos and emits machine-readable results next to ours. From then on the comparative scorecard (ours vs external comparator) is produced automatically by benchmark-run and committed — the Stage 4 parity table then assembles itself from accumulated data.
