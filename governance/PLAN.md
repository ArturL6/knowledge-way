...
| `benchmark-run` | every 20 minutes | Once the Stage 0 harness exists, run it when integration changes and commit scorecards. Once scripted, compare external comparator on the same tasks; flag regressions for Sol. |
...
**Goal:** Make retrieval quality measurable before improving it, and learn from external comparator. Extends the existing `benchmarks/` folder (task/corpora/result schemas already exist — build on them).
...
- **0.4 — external comparator test drive.** Run external comparator (github.com/abhigyanpatwari/external comparator) on the same workspace; execute the 12 question classes in Appendix A.7; record correct/partial/incorrect/not-representable + latency; write `benchmarks/external-comparator-findings.md`. License: third-party licensing — study concepts, never copy code.
...
- [ ] external comparator findings documented with representation-gap vs accuracy-gap classification.
...
**Goal:** `workspace_dependencies` stops being manually declared and becomes extracted, evidence-backed knowledge; the workspace reads as one application. This is the external comparator-groups capability we are testing against.
...
- **4.4 — Ablation benchmark.** Drive a real coding agent over the gold tasks in configurations: **A** grep-only; **B** A + lexical/symbol tools; **C** B + semantic/cards; **D** C + graph/impact; **E** external comparator (Stage 0 setup). Metrics: tool calls, tokens, files opened, time-to-first-correct-file, cross-repo impacts found, false leads.
...
8. **external comparator: study concepts, never copy code** (third-party licensing).
...
## A.6 external comparator positioning

external comparator (repo: abhigyanpatwari/external comparator) already implements much repository-level intelligence: tree-sitter parsing with language-aware resolution, knowledge graph, BM25 + embeddings + RRF, process discovery, impact traversal, MCP, and repository groups with a Contract Registry for cross-repo links. It validates our hybrid-retrieval assumptions and is the benchmark baseline (config E). Our differentiation lives **above** that layer: a unified multi-repo application graph, persistent versioned cards, a stronger evidence/completeness model, freshness, agent overlays, and eventually runtime evidence. **License: third-party licensing — behaviors and public architecture may be studied and benchmarked; source code must never be copied or mechanically rewritten.**

## A.7 external comparator evaluation question classes (used in packet 0.4)
