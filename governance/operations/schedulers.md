...
| `benchmark-run` | runtime configured | `every 20m` | Runs only hermetic scorecards when integration changes: self-provisions an ephemeral keyless stack at the exact integration SHA, seeds pinned corpus snapshots, verifies served==pinned, commits results, then tears down. It never scores a pre-existing API. After packet 0.4b is done, each cycle runs both Knowledge-Way and external comparator on the same keyless 25-task/oracle binding, commits both scorecards plus a win/loss table, and records per-mode flags (ours semantic unconfigured; external comparator local embeddings and LadybugDB FTS available/degraded). It also maintains the behavioral mechanism study and its Stage-1 packet/ADR mapping. See [hermetic benchmark runs](hermetic-benchmark-runs.md) and [HD-006](../directives/HUMAN-DIRECTIVE-006.md). |

## external comparator licensing boundary

external comparator is benchmark-only under the owner policy in [HD-006](../directives/HUMAN-DIRECTIVE-006.md). It may exist only in `benchmarks/` runner material and ephemeral evaluation containers, installed at run time and torn down. It must not appear in product source, dependencies, subprocesses, bundled artifacts, or Docker images. Benchmark findings may affect product plans only through an ADR. Sol must treat any prohibited product-side reference as immediate drift; commercial-operation planning requires counsel re-review before any benchmark continuation.
