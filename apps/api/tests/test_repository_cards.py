...
def test_repository_card_is_bounded_commit_scoped_and_uses_only_persisted_evidence():
    ...
    db.add_all([
        ...
        File(id="stale-file", repository_id="repo", path="stale.py", language="python", content="x", content_hash="stale-file-hash", size_bytes=1, indexed_commit_sha="c" * 40),
    ...
    db.add_all([
        ...
        Symbol(id="stale-symbol", repository_id="repo", file_id="stale-file", name="stale", qualified_name="stale", symbol_type="function", language="python", start_line=1, end_line=1, start_byte=0, end_byte=1, source_text="x"),
    ...
    db.add_all([
        ...
        CodeCard(id="stale-card", repository_id="repo", symbol_id="stale-symbol", source_hash="stale-symbol-hash", indexed_commit_sha="c" * 40, model="test", prompt_version="v1", status="ready", summary="Stale evidence.", details={"keywords": ["stale"]}),
        ...
        StructuralCard(id="stale-module", repository_id="repo", kind="package", path="stale", facts={"direct_files": [{"path": "stale.py"}], "languages": {"python": 1}, "boundary_edges": [{"count": 9}]}, content_fingerprint="stale-content", provenance_fingerprint="stale-provenance", indexed_commit_sha="c" * 40, schema_version="v1"),
    ])
    db.commit()
    ...
    try:
        ...
        assert "Stale evidence" not in card["retrieval_document"]
        assert all(module["path"] != "stale" for module in card["facts"]["modules"])
