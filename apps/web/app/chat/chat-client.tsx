...
export default function ChatClient() {
  ...
  return <section className="chat"><h2>Grounded code questions</h2><p className="muted">Retrieval is repository- and commit-scoped. Answers are evidence summaries, not unverified model claims.</p>
    <form onSubmit={ask}><label>Repository<select value={repositoryId} onChange={(event) => setRepositoryId(event.target.value)} required><option value="">Choose an indexed repository</option>{repositories.map((repository) => <option key={repository.id} value={repository.id}>{repository.name}{repository.indexed_commit_sha ? ` · ${repository.indexed_commit_sha.slice(0, 12)}` : ''}</option>)}</select></label><label className="visually-hidden" htmlFor="code-question">Question about this codebase</label><textarea id="code-question" rows={4} value={q} onChange={(event) => setQ(event.target.value)} placeholder="Where is authentication implemented?"/><p><button disabled={loading || !repositoryId}>{loading ? 'Retrieving…' : 'Ask codebase'}</button></p></form>
