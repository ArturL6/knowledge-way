...
export default function GraphExplorer() {
  ...
  return <>
    ...
    <section className="card graph-card">
      ...
      {loading ? <div className="graph-state" role="status">Requesting graph data…</div> : filteredGraph.nodes.length > 0 ? <><div className="graph-hint">Tip: labels are clickable too. Dragging pins a node so dense graphs stay readable.</div><GraphCanvas data={filteredGraph} onNodeClick={setSelected} selectedNodeId={selected?.id} /></> : <div className="graph-state"><strong>No graph data to display.</strong><p className="muted">{source === 'empty' ? 'Choose a repository and symbol to load a graph, or load the fixture demo.' : graph.links.length > 0 ? 'The selected filters returned no connected nodes. Adjust filters or load the fixture demo.' : 'This selection returned no graph relationships.'}</p></div>}
    </section>
